...
namespace detail
{
	...
	struct _interpreter
	{
		...
		/* For now, _interpreter is implemented as a singleton since its
		   ...
		   separate process for each. [1] Furthermore, many python objects
		   expect that they are destructed in the same thread as they were
		   constructed. [2] So for advanced usage, a `kill()` function is
		   provided so that library users can manually ensure that the
...
namespace detail
{
	...
	// template <> struct select_npy_type<long long> { const static NPY_TYPES
	// type = NPY_INT64; }; static_assert(sizeof(unsigned long long) == 8);
                  ...
                  const std::map<std::string, std::string> & keywords)
{
	...
	PyObject * res = PyObject_Call(
	    detail::_interpreter::get().s_python_function_fill_between,
	    args,
	    kwargs);
